...
static void registery_fake_gui_callbacks(struct cr_plugin *ctx) {
    if (!g_plugin_mouse_cb) {
        ...
        // Get a protected closure for the symbol
        cb_mouse_event_func mouse_cb = (cb_mouse_event_func)cr_symbol_get_closure(g_plugin_mouse_cb);
        // Use the trampoline from the Host to unwrap the `cr_closure` and all the callback function.
        fake_register_mouse_events(mouse_cb, ctx);
    }
    if (!g_plugin_key_cb) {
        // Only need one instance of `cr_symbol` for each callback function.
        ...
        cb_key_event_func key_cb = (cb_key_event_func)cr_symbol_get_closure(g_plugin_key_cb);
        // Use the trampoline from the Host to unwrap the `cr_closure` and all the callback function.
        fake_register_key_events(key_cb, ctx);
    }
}
...
//
CR_EXPORT int plugin_mouse_events(void *userdata, double x, double y, int buttons) {
    struct cr_plugin *ctx = (struct cr_plugin *)userdata;
    fprintf(stdout, "cb_guest: mouse event callback. (x=%f,y=%f,buttons=0x%x): ver=%d\n",
